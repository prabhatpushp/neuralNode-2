...
used_nodes.append(input_node1)
used_nodes.append(input_node2)

hidden_nodes = []

for i in range(1, 4):
    hidden_node = NeuralNode(name=f'Hidden{i}', activation_function=sigmoid)
    incoming_nodes = random.sample(used_nodes, min(4, len(used_nodes)))
    for node in incoming_nodes:
        hidden_node.add_incoming_node(node)
    hidden_nodes.append(hidden_node)
    used_nodes.append(hidden_node)

final_hidden_nodes = []

for i in range(1, 4):
    hidden_node = NeuralNode(name=f'FinalHidden{i}', activation_function=tanh)
    incoming_nodes = random.sample(hidden_nodes, min(4, len(hidden_nodes)))
    for node in incoming_nodes:
        hidden_node.add_incoming_node(node)
    final_hidden_nodes.append(hidden_node)
    used_nodes.append(hidden_node)
...
for node in final_hidden_nodes:
    output_node1.add_incoming_node(node)
    output_node2.add_incoming_node(node)
